...
original_image = "./Images/5_salt_pepper.png"
original = cv2.imread(original_image, cv2.IMREAD_GRAYSCALE)

assert original is not None

kernel_size = 5
filtered = median_filter(original, kernel_size)
...
#         filtered = median_filter(original, ksize)
...
#     plt.show()
#
#     filtered = median_filter(original,3)
#
...
assert morph.any()
if DEBUG:
    cv2.imwrite(f'./Report/salt_n_pepper/final_{kernel_size}.png', final)
    cv2.imwrite(f'./Report/salt_n_pepper/morph_{kernel_size}.png', morph)
